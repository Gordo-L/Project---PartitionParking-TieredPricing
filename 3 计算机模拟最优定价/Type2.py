...
for j in range(n):
    for i in range(m):
        #每隔50次，存储一次Cell矩阵，如下所示：
        if i%50 == 0:
            np.savetxt("T2_Cell"+str(i//50)+".csv",Cell,fmt='%d',delimiter=',')
        ...
        if a<distance[i,j] and distance[i,j]<b:#（排除极端情况）#if 这里有车才能进行:
            ...
            vector2 = Event[i,j,1]
            if flag == 1:
                if Omega[int(vector2[0]),int(vector2[1])] <= 2 and Omega[int(vector2[0]),int(vector2[1])] >= 0:  #（终点在p区内）
